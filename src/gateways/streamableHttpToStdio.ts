...
export async function streamableHttpToStdio(args: StreamableHttpToStdioArgs) {
  ...
  mcpTransport.onerror = (err: Error) => {
    ...
    try {
      if (!mcpClient) {
        if (message.method === 'initialize') {
          mcpClient = newInitializeMcpClient({ message })

          const originalRequest = mcpClient.request.bind(mcpClient)
          mcpClient.request = async function (
            possibleInitRequest,
            ...restArgs
          ) {
            if (
              InitializeRequestSchema.safeParse(possibleInitRequest).success &&
              message.params?.protocolVersion
            ) {
              possibleInitRequest.params!.protocolVersion =
                message.params.protocolVersion
            }
            result = await originalRequest(possibleInitRequest, ...restArgs)
            return result
          }
      ...
      process.stdout.write(
        JSON.stringify(
          wrapResponse(req, { error: { code: errorCode, message: errorMsg } }),
        ) + '\n',
      )
      return
    }
